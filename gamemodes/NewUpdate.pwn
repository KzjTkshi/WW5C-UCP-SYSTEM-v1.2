/*
		...
		- Added Anti CHENH4X
		- Switch Name u1.0 to w1.2
		- Added Command /LOCKSERVER

		[ STAGE UPDATE ]

		- Added NEW TEAM ( SEGERA )
		- Added Vip ( SEGERA )
		...
		- GUNAKAN GAMEMODE INI SEBAIK MUNGKIN JANGAN PERNAH DI JUAL, KALIAN MENDAPATKAN GAMEMODE SECARA GRATIS TANPA MEMBELI
		TOLONG HARGAI PENGEMBANG GAMEMODE INI ( K_ZxZ Atau Saya Sendiri Yang Mengetik Pesan Ini. )

		- GAMEMODE INI SECARA GRATIS BISA KALIAN MILIKI. JIKA INGIN MENGUPDATENYA LAGI SILAKAN SAYA IZINKAN TAPI JIKA GAMEMODE INI DI JUAL KALIAN TELAH MELANGGAR HAK CIPTA.
...
#define SendSyntaxMessage(%0,%1) \
	SendClientMessageEx(%0, COLOR_WHITE, "KEGUNAAN:{FFFFFF} "%1)
...
Database_Connect()
{
	...
	{
	    print("[MySQL] - Gagal Terkoneksi!");
	}
	else
	{
		print("[MySQL] - Terkoneksi!");
		SetGameModeText("WW5Country w1.2");
	}
}
...
WW5C::LoadCharacterData(playerid)
{
    ...
    SendServerMessage(playerid, "Akunmu tersimpan di databse 'Survivors'!");
    return 1;
}
...
WW5C::OnPlayerPasswordChecked(playerid, bool:success)
{
	...
	if(!success)
	{
		...
		{
		    SendServerMessage(playerid, "Kamu telah salah memasukan password sebanyak 3 kali!");
		    KickEx(playerid);
			return 1;
		}
	}
...
WW5C::InsertPlayerName(playerid, name[])
{
	...
	if(count > 0)
	{
        ShowPlayerDialog(playerid, DIALOG_MAKECHAR, DIALOG_STYLE_INPUT, "Create Character", "ERROR:Nama ini sudah digunakan oleh pemain lain!", "Create", "Back");
	}
	else
	{
		...
		format(WorldWar5CountryData[playerid][pName], MAX_PLAYER_NAME, name);
	 	ShowPlayerDialog(playerid, DIALOG_AGE, DIALOG_STYLE_INPUT, "Character Age", "Masukan Umur Karaktermu", "Continue", "Batal");
	}
	return 1;
}
...
//PROJECT BARU
//    BRITAN_BASE = GangZoneCreate(-516,2112,-288,2298); // Ghosts Town
...
public OnPlayerConnect(playerid)
{
	...
	GangZoneShowForPlayer(playerid, BRITAN_BASE, COLOR_BRIGHTRED);
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    ...
    if(dialogid == DIALOG_CHARLIST)
    {
		if(response)
		{
			if (SurvivorsChar[playerid][listitem][0] == EOS)
				return ShowPlayerDialog(playerid, DIALOG_MAKECHAR, DIALOG_STYLE_INPUT, "Create Character", "Masukan Nama KarakterMu", "Create", "Exit");
	...
	if(dialogid == DIALOG_MAKECHAR)
	{
	    if(response)
	    {
		    if(strlen(inputtext) < 1 || strlen(inputtext) > 24)
				return ShowPlayerDialog(playerid, DIALOG_MAKECHAR, DIALOG_STYLE_INPUT, "Create Character", "Masukan Nama KarakterMu", "Create", "Back");
	...
	if(dialogid == DIALOG_AGE)
	{
		if(response)
		{
			if(strval(inputtext) >= 60)
			    return ShowPlayerDialog(playerid, DIALOG_AGE, DIALOG_STYLE_INPUT, "Character Age", "ERROR: Tidak boleh lebih dari 60 tahun!", "Continue", "Batal");

			if(strval(inputtext) < 15)
			    return ShowPlayerDialog(playerid, DIALOG_AGE, DIALOG_STYLE_INPUT, "Character Age", "ERROR: Tidak boleh di bawah 15 Tahun!", "Continue", "Batal");

			WorldWar5CountryData[playerid][pAge] = strval(inputtext);
			ShowPlayerDialog(playerid, DIALOG_ORIGIN, DIALOG_STYLE_INPUT, "Character Origin", "Silakan masukkan Asal Karakter Anda(United States of America,Great Britan,Indonesia,Uni Soviet,Saudi Arabia,Germany):", "Continue", "Quit");
		}
		else
		{
	...
	{
	    if(!response)
	        return ShowPlayerDialog(playerid, DIALOG_ORIGIN, DIALOG_STYLE_INPUT, "Character Origin", "Silakan masukkan Asal Karakter Anda(United States of America,Great Britan,Indonesia,Uni Soviet,Saudi Arabia,Germany):", "Continue", "Quit");
...
public OnPlayerClickMap(playerid, Float:fX, Float:fY, Float:fZ)
{
    ...
    return SendClientMessage(playerid,COLOR_WHITE, "Diteleportasi ke lokasi yang ditandai.");
}
...
{
	new msg[400];
	format(msg, 400, "{09FF00}%s • Penambahan Command Untuk Admin\n",msg);
	format(msg, 400, "{09FF00}%s • Penambahan ANTICHENH4X\n",msg);
	format(msg, 400, "{09FF00}%s • Penambahan TEAM ( SEGERA )\n", msg);
	format(msg, 400, "{09FF00}%s • Perubahan Versi Server Dari u1.0 --> u1.2\n", msg);
	format(msg, 400, "{09FF00}%s • -------------\n\n", msg);
	format(msg, 400, "{FF0000}%s • -------------", msg);
...
CMD:givecash(playerid, params[])
{
	...
	if(WorldWar5CountryData[playerid][pAdmin] < 7)
		return SendErrorMessage(playerid, "Anda tidak memiliki hak istimewa untuk menggunakan perintah ini!");
	...
	AdminSendMessage(COLOR_WHITE, "AdmCmd: %s has give $%s cash to %s", WorldWar5CountryData[playerid][pUCP], FormatNumber(strcash(amount)), GetName(targetid));
...
{
	new String[10000];
	if(WorldWar5CountryData[playerid][pAdmin] < 2) return SendClientMessage(playerid, COLOR_GREY, "ERROR: Anda tidak memiliki hak istimewa untuk menggunakan perintah ini!");
    if(ServerLocked)
    {
        ServerLocked = false;
...
CMD:settime(playerid, params[])
{
	...
	if(params[0] < 0 || params[0] > 24)
		return SendClientMessage(playerid, -1, "Waktu dalam sehari hanya dapat diatur dari 0 hingga 24");
...
CMD:slap(playerid, params[])
{
    ...
    if (userid == INVALID_PLAYER_ID)
	    return SendErrorMessage(playerid, "PLAYER ID TIDAK BENAR!");
	...
	SendServerMessage(playerid, "Anda Menampar %s", ShowName(userid));
	return 1;
}
...
CMD:spec(playerid, params[])
{
	...
	if (sscanf(params, "u", userid))
		return SendSyntaxMessage(playerid, "/spec(tate) [playerid/PartOfName] - Ketik \"/unspec\" untuk berhenti menonton.");

	if (userid == INVALID_PLAYER_ID)
	    return SendErrorMessage(playerid, "PLAYERID TIDAK BENAR.");
	...
	SendServerMessage(playerid, "Anda Sekarang Menonton %s (ID: %d).", GetName(userid), userid);
...
CMD:unspec(playerid, params[])
{
    ...
    if (GetPlayerState(playerid) != PLAYER_STATE_SPECTATING)
		return SendErrorMessage(playerid, "Anda Tidak Menonton Player 1 Pun.");

    PlayerSpectatePlayer(playerid, INVALID_PLAYER_ID);
    PlayerSpectateVehicle(playerid, INVALID_VEHICLE_ID);

    TogglePlayerSpectating(playerid, false);
    SendServerMessage(playerid, "Anda Dalam mode Lama Menonton.");
    if(WorldWar5CountryData[playerid][pAduty])
    {
		SetPlayerColor(playerid, COLOR_RED);
	}
...
CMD:setarmor(playerid, params[])
{
	...
	if (WorldWar5CountryData[playerid][pAdmin] < 5)
	    return SendErrorMessage(playerid, "Anda tidak memiliki hak istimewa untuk menggunakan perintah ini.");
	...
	SendServerMessage(playerid, "Anda mengatur %s's armor %.", GetName(userid), amount);
	return 1;
}
...
CMD:sethp(playerid, params[])
{
	...
	SendServerMessage(playerid, "Anda Mengatur %s's ke %.", GetName(userid), amount);
	return 1;
}
...
CMD:kick(playerid, params[])
{
    ...
    if (WorldWar5CountryData[userid][pAdmin] > WorldWar5CountryData[playerid][pAdmin])
	    return SendErrorMessage(playerid, "Pemain yang ditentukan memiliki Peringkat yang lebih tinggi.");

	SendClientMessageToAllEx(COLOR_BLUE, "AdmCmd: %s Telah di kick oleh %s ALASAN: %s.", GetName(userid), WorldWar5CountryData[playerid][pUCP], reason);
	KickEx(userid);
	return 1;
}
...
{
	SendClientMessage(playerid, -1, "Command ini tidak memiliki izin di server ini!");
	KickEx(playerid);
    return 1;
}
